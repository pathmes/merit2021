...
p1 <- ggplot(data = df10, mapping = 
               aes(x = University, y = utenths_p, 
                   fill = Tenths)) + 
  geom_bar(stat = "identity") +
  scale_y_continuous(breaks = seq(0, 100, 10)) +
  labs(y = "Cumulative percent of Students")

p1 + scale_fill_continuous_divergingx(palette = "Geyser")
p1 + scale_fill_gradient2(low="red", mid="yellow", high="green", 
                          midpoint = -5)

# The default ordering of the universities is distracting.  
# Better to order the change the order based on some information in the merit list.
# Order the universties according to the merit position of the middle student of
# each university
...
p21 <- p2 + scale_fill_gradient2(low="brown", mid="yellow", high="green",
midpoint = -5) +  theme(legend.position = "none") 

# Not a pleasing color scheme - try 'Geyser'
# Increase the visibility of the grid lines by making the background darker 
p2 + scale_fill_continuous_divergingx(palette = "Geyser", 
                                      mid = -5, rev = TRUE) +
  theme(panel.background = element_rect(fill = "gray86"))

p2 + scale_fill_continuous_divergingx(palette = "Geyser",
mid = -5, rev = TRUE) +
theme(panel.background = element_rect(fill = "gray70"), legend.position = "none")
